...
source("_code/function-lib/fun_process-hand-made-operations-file2.R")
key <- read_csv("_data/tidy/td_year-plot-trt-key.csv")

#--make it more manageable for trouble shooting
...
praw <- read_excel("_data/raw/Marsden-activites-for-apsim-noplots.xlsx", 
...
pap <- 
  ...
  mutate(sds_ac = ifelse(crop == "oats", sds_lbsac * oats_sds_lb, 
                         ifelse (crop == "red clover", sds_lbsac * redclover_sds_lb,
                                 ifelse (crop == "lucerne", sds_lbsac * alf_sds_lb, sds_ac)))) %>% 
   
  #--plant populations
  mutate(
    #--sds_ac (everything) go to plants/m2 (this is the planting density)
    plants_m2 = sds_ac * ac_ha * 1/10000,
    # if there are stand counts plants/m2 (oat, clover, alf), change plants/m2 to those
         plants_m2 = ifelse(!is.na(stand_counts_plants_m2), 
                            stand_counts_plants_m2, 
                            plants_m2),
    # if there are stand counts plants/acre (corn, soy), change plants/m2 to those
         plants_m2 = ifelse(!is.na(stand_counts_plants_acre),
